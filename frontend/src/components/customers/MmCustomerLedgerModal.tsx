...
import { formatCurrency, billStatusVariant, type Bill, type BillStatus, type MmCustomer } from '@/types';
import { formatDate } from '@/lib/utils';
import { useClosingTransition } from '@/hooks/use-closing-transition';

interface MmCustomerLedgerModalProps {
  customer: MmCustomer;
  onClose: () => void;
}

/** MM billing module's own customer ledger — mirrors CustomerLedgerModal but
 * against mmCustomersApi and MmBillDetailModal. */
export function MmCustomerLedgerModal({ customer, onClose }: MmCustomerLedgerModalProps) {
  const [selectedBill, setSelectedBill] = useState<Bill | null>(null);
  const { closing, requestClose } = useClosingTransition(onClose);

  const { data: bills, isLoading } = useQuery({
    queryKey: ['mm-customers', customer.id, 'bills'],
    queryFn: () => mmCustomersApi.getBills(customer.id),
  });
          ...
          <div className="overflow-auto flex-1 p-6 space-y-4">
            ...
            <div>
              ...
              {!isLoading && (bills?.length ?? 0) === 0 && (
                <p className="text-center text-muted-foreground py-8 text-sm">No MM bills for this customer yet.</p>
              )}
              {!isLoading && (bills?.length ?? 0) > 0 && (
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b text-muted-foreground text-xs">
                      <th className="text-left py-2 font-medium">MM Bill No</th>
