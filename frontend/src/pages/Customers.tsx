...
const EMPTY: Partial<Customer> = { name: '', phone: '', email: '', address: '', gstin: '', notes: '' };
...
export default function CustomersPage() {
  ...
  // from the URL so a refresh doesn't re-trigger the highlight.
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => customersApi.list({ search, page, limit: LIMIT }),
    placeholderData: (prev) => prev,
  });

  const createMutation = useMutation({
    mutationFn: (data: Partial<Customer>) => customersApi.create(data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['customers'] }); setEditing(null); toast({ title: 'Customer created', variant: 'success' }); },
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<Customer> }) => customersApi.update(id, data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['customers'] }); setEditing(null); toast({ title: 'Customer updated', variant: 'success' }); },
  });
  ...
  // Scroll to and highlight the row whose id matches ?id=… in the URL.
  useEffect(() => {
    if (!highlightId) return;
    const row = document.getElementById(`customer-${highlightId}`);
    if (row) {
    ...
    // run when the list or highlight id changes (pagination may have to load
    // a different page before the target row is mounted)
