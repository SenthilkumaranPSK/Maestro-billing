...
// MM billing module's own GST Report — mirrors GstReport.tsx, scoped to
// series='MM'. Kept entirely separate from the studio's regular GST filing
// since MM has its own bill-numbering series and product catalog.
export default function MmGstReportPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['bills', 'mm-gst-report', from, to],
  ...
  const handleExportCsv = () => {
    const lines: string[] = [];
    lines.push(`MM GST Summary,${displayMonth}`);
    lines.push('');
    ...
    lines.push('');
    lines.push(`MM Bills,${bills.length}`);
    ...
    a.href = url;
    a.download = `MM-GST-Report-${month}.csv`;
    a.click();
    ...
    <div className="space-y-5 max-w-3xl">
      ...
      {truncated && (
        <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          This month has {data!.meta.total} MM bills but only {data!.data.length} could be loaded —
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <Card>
          <CardContent className="pt-4 pb-4">
            <p className="text-xs text-muted-foreground uppercase tracking-wide">MM Bills</p>
