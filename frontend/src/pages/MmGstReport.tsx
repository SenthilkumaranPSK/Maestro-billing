...
// MM billing module's own GST Report — same bill-wise, HSN-wise CSV export
// as the main studio's GstReport.tsx (identical column template, "SGCT"
// typo included), scoped to series='MM' and MmCustomer instead of Customer.
export default function MmGstReportPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['bills', 'mm-gst-detail-report', from, to],
  ...
  // One row per (bill, HSN code) — same grouping as the MM/A4 Tax Invoice's
  // own HSN summary table (lib/hsnSummary.ts), so the two can never disagree.
  ...
  const handleExportCsv = () => {
    const lines: string[] = [];
    // Header text matches the same client template GstReport.tsx uses,
    // "SGCT" typo included — the underlying field is still SGST throughout.
    ...
    rows.forEach(({ bill, group: g }, i) => {
      ...
      lines.push(
        [
          ...
          csvEscape(bill.buyerName || bill.mmCustomer?.name || ''),
          csvEscape((bill.buyerName ? bill.buyerGstin : bill.mmCustomer?.gstin) ?? ''),
          csvEscape(g.hsnSac),
    ...
    a.href = url;
    a.download = `MM-GST-Report-${from}_to_${to}.csv`;
    a.click();
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold">MM GST Report</h2>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <p className="text-xs text-muted-foreground">
            One row per MM bill per HSN/SAC code — S.No, Bill No, Date, Name, GSTIN, HSN, Taxable Value, IGST,
          ...
          {truncated && (
            <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800">
              This range has {data!.meta.total} MM bills but only {data!.data.length} could be loaded —
      ...
      <p className="text-xs text-muted-foreground">
        ...
        Figures cover all non-cancelled MM bills. Verify with your CA before filing.
      </p>
    </div>
  );
}
