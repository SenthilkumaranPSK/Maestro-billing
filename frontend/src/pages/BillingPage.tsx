...
export default function BillingPage() {
  ...
  /**
   * The ONE WhatsApp send path for this page — used by both the "Send Bill on
   * WhatsApp" button and the "Send on WhatsApp after saving" checkbox.
   *
   * Takes the bill as an argument rather than reading `savedBill` from state,
   * because the checkbox path runs inside the create mutation's `onSuccess`,
   * whose closure still sees the pre-save `savedBill` (null).
   *
   * The bill goes as a MESSAGE, not a PDF. Attaching the PDF meant driving
   * WhatsApp's attach menu, file input and preview, and it broke outright —
   * see lib/whatsappBillText.ts. A text message is placed in the composer by
   ...
   */
  const shareBillOnWhatsApp = async (bill: Bill, phone: string) => {
    const text = buildWhatsAppBillText(bill, settings ?? {});

    // Preferred path: the desktop app commits the message itself. Showing the
    // panel first both mounts the <webview> the send needs and lets the
    // operator watch it happen rather than staring at a spinner.
    if (isWhatsAppTextSendAvailable()) {
      ...
      try {
        ...
        // Never claim a send that didn't happen — the message is left typed in
        // the chat, so finishing it is one tap.
        toast({
          title: 'Ready to send',
          description: `${err instanceof Error ? err.message : 'Could not send automatically'} — press Send in the chat to finish.`,
          variant: 'destructive',
        });
        return;
      } finally {
    ...
    // No automation available: open the chat with the bill already typed.
    if (isWhatsAppEmbedAvailable()) {
      requestWhatsAppChat({ phone, caption: text });
      navigate('/whatsapp');
    } else {
      // Plain browser / two-PC client — real web.whatsapp.com in a new tab.
      openWhatsAppChatInNewTab(phone, text);
    }

    toast({
      title: 'Chat opened',
      description: `Bill ${bill.billNumber} is typed and ready — press Send.`,
      variant: 'success',
    });
  };
    ...
    onSuccess: async (bill) => {
      ...
      if (sendOnWhatsApp && customer.phone.trim()) {
        if (!isValidIndianPhone(customer.phone.trim())) {
          ...
          await shareBillOnWhatsApp(bill, customer.phone.trim());
        }
      }
    },
    onError: (err: Error) => {
  ...
  /** "Send Bill on WhatsApp" after a bill is saved — see shareBillOnWhatsApp. */
  const handleWhatsAppShare = async () => {
    ...
    await shareBillOnWhatsApp(savedBill, customer.phone.trim());
  };
