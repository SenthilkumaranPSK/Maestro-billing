            ...
            <table className="w-full">
              <thead>
                <tr className="border-b bg-slate-50/80">
                  {['MM Bill No', 'Date', 'Customer', 'Items', 'Amount', 'Payment', 'Status', 'Actions'].map((h) => (
              ...
              <tbody className="stagger-children">
                ...
                {!isLoading && data?.data.length === 0 && (
                  <tr>
                    <td colSpan={8} className="py-10 text-center text-muted-foreground text-sm">No MM bills found</td>
                  </tr>
                )}
                {data?.data.map((bill) => (
